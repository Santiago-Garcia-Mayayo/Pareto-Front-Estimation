...
from problems import dtlz3_f1, dtlz3_f2, dtlz3_f3
import time

# Multimodal+Convex/ doesnt work
...
funcs = [dtlz3_f1, dtlz3_f2, dtlz3_f3]
...
for i in range(MAX_ITER):
    ...
    ax.plot_wireframe(
        x_sph, y_sph, z_sph, color="gray", alpha=0.2, rstride=2, cstride=2
    )

    pts = opt.pareto_front_y

    visible_pts = pts[np.all(pts < LIMIT_MAX, axis=1)]
    ...
    ax.scatter(
        ...
        c="red",
        s=30,
        label=f"Visible Pts: {len(visible_pts)}",
    )
    ...
    ax.set_ylabel("f2")
    ax.set_zlabel("f3")
    ax.set_title(f"DTLZ3 - Iter {i+1} (Fixed View [0, {LIMIT_MAX}])")
