...
from problems import dtlz2_f1, dtlz2_f2, dtlz2_f3
import time

# Convex Pareto Front/ the method works good
time_start = time.time()
N_OBJS = 3
k = 10
N_VARS = N_OBJS + k - 1
bounds = [(0.0, 1.0)] * N_VARS
funcs = [dtlz2_f1, dtlz2_f2, dtlz2_f3]
...
unique_start = len(np.unique(np.round(opt.pareto_front_y, 4), axis=0))
time_end = time.time()
...
for i in range(MAX_ITER):
    ...
    print(f"  > Expected Hypervolume Gain: {expected_gain:.6f}")

    if expected_gain < HV_TOLERANCE:
        print("  > [STOP] Gain is negligible (Convergence reached).")
        break
    ...
    ax.set_ylabel("f2")
    ax.set_zlabel("f3")
    ax.set_title(f"DTLZ2 Infill (Sphere) - Iter {i+1}")
    ax.legend()
